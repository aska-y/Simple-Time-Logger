...
class LoginView(TokenObtainPairView):
    """
    ユーザーがログインする際にアクセストークンとリフレッシュトークンを発行。
    POSTリクエスト時に有効な認証情報が供された場合、レスポンスにアクセストークンとリフレッシュトークンを含める。
    さらにこれらのトークンをCookieに設定してクライアントに送り返す。
    Cookieの設定には、ドメイン、パス、有効期限、セキュアフラグ、HttpOnlyフラグ、SameSiteフラグなどのパラメータを含む。
    """
...
class LogoutView(APIView):
    """
     ユーザーのログアウト時にアクセストークンとリフレッシュトークンのCookieを削除。
    """
...
class TokenVerifyView(APIView):
    """
    提供されたアクセストークンが有効であるかどうかを検証。
    クライアントから送信されたCookie内のアクセストークンを取得して検証。
    トークンが無効な場合は、エラーメッセージとともに400 Bad Requestエラーを返す。
    """
...
class RefreshTokenView(APIView):
    """
    リフレッシュトークンを使用して新しいアクセストークンとリフレッシュトークンを発行する。
    送信されたリフレッシュトークンを検証し、有効であれば新しいトークンを生成し、Cookieに設定して返す。
    ユーザーはログイン状態を維持しつつ、トークンを定期的に更新することができるようになる。
    """
